...
def show(img,title = 'image',usePIL=True):
    if usePIL:
        if len(img.shape) == 3:
...
def disSSIM(orig,test,showResults=True,threshold = 1,drawImg = None):
...
#    (score, diff) = compare_ssim(grayA, grayB, full=True, use_sample_covariance =  False, sigma= 25.7, gaussian_weights= True) #  greater the signma value greater the accuracy

    (score, diff) = compare_ssim(grayA, grayB, full = True, use_sample_covariance = False, sigma= 1.5, gaussian_weights= True)
    ...
    save(testImg,'SSIM_result_test_Image')
    save(origImg,'SSIM_result_original_Image')
...
def checkinrange(j,i,h,w,startpoints,endpoints):
    for spoint,epoint in zip(startpoints,endpoints):
        sj,si=spoint
        ej,ei=epoint
        if (i+w) in range(si,ei):
            if j in range(sj,ej):
                return True
            elif (j+h) in range(sj,ej):
                return True
        elif i in range(si,ei):
            if j in range(sj,ej):
                return True
            elif (j+h) in range(sj,ej):
                return True
...
def checknear(j,i,h,w,startpoints,endpoints,hbias,wbias):
    for spoint,epoint in zip(startpoints,endpoints):
        sj,si=spoint
        ej,ei=epoint
        sj=sj-hbias
...
def isConnected(rec1, rec2, checkOffset=0, debugLog = True, dualOffset = 0):
    rec1_stpt, rec1_endpt = rec1[0], rec1[1]
    if debugLog:
        print('*******checking connections***********')
        print('rec1', rec1)
        print('rec2', rec2)
    rec2_stpt = (rec2[0][0] + dualOffset, rec2[0][1] + dualOffset)
    rec2_endpt = (rec2[1][0] + dualOffset, rec2[1][1] + dualOffset)
    if (rec1_stpt[0]+checkOffset in range(rec2_stpt[0], rec2_endpt[0], 1)) or \
            (abs(rec1_stpt[0]-checkOffset) in range(rec2_stpt[0], rec2_endpt[0], 1)) or \
            (rec1_endpt[0]+checkOffset in range(rec2_stpt[0], rec2_endpt[0], 1)) or \
            (abs(rec1_endpt[0]-checkOffset) in range(rec2_stpt[0], rec2_endpt[0], 1)):
        if (rec1_stpt[1]+checkOffset in range(rec2_stpt[1], rec2_endpt[1], 1)) or \
                (abs(rec1_stpt[1]-checkOffset) in range(rec2_stpt[1], rec2_endpt[1], 1)) or \
                (rec1_endpt[1]+checkOffset in range(rec2_stpt[1], rec2_endpt[1], 1)) or \
                (abs(rec1_endpt[1]-checkOffset) in range(rec2_stpt[1], rec2_endpt[1], 1)):
            print('connected')
            return True

    if checkOffset != 0: # if does'nt satisfies user's offset try with '0' offset
        return isConnected(rec1,rec2,debugLog=False, dualOffset=dualOffset)

    print('not connected')
    return False


def findResultants(start_points,end_points, proximity = 10):
    startpoints = start_points.copy()
    endpoints = end_points.copy()
    print('****@find resultants****')
    for startpoint_this, endpoint_this in zip(startpoints.copy(), endpoints.copy()):
        thisRect = (startpoint_this, endpoint_this)
        print(startpoints)
        for startPoint_among_all, endPoint_among_all in zip(startpoints.copy(), endpoints.copy()):
            # compare this rect with all the others
            if (startPoint_among_all, endPoint_among_all) != (startpoint_this, endpoint_this):
                # avoid comparing rect with itself
                if isConnected(thisRect, (startPoint_among_all, endPoint_among_all), checkOffset=proximity):
                    # find resultant of both
                    res_this_st = min(startPoint_among_all, startpoint_this, endPoint_among_all, endpoint_this)
                    res_this_end = max(startPoint_among_all, startpoint_this, endPoint_among_all, endpoint_this)
...
def minima(val1,val2):
    if val1[0]+val1[1] < val2[0]+val2[1]:
        return val1
    else:
        return val2

def maxima(val1,val2):
    if val1[0]+val1[1] > val2[0]+val2[1]:
        return val1
    else:
        return val2

def findRecSeries(step_size,box_steps,rec_init,start_points, end_points):
    for box in range(box_steps):
        start_points = [i for i in start_points if i in range(i[0]+ rec_init[0] + step_size) or i in range(i[1]+rec_init[1]+step_size)]